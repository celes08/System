...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['saveAccountChanges'])) {
        ...
        if (empty($errorMsg)) {
            $updateStmt = $con->prepare("UPDATE signuptbl SET first_name = ?, last_name = ?, department = ?, date_of_birth = ?, username = ? WHERE user_id = ?");
            $updateStmt->bind_param("sssssi", $newFirstName, $newLastName, $newDepartment, $newDateOfBirth, $newUsername, $userId);
